...
if __name__ == '__main__':
    ...
    # criterion
    dataset = Chain1Dataset("data/test.csv", alphabet, N_tot, M_testing, snr_dB=0)
    Nb_trials = len(dataset)
    mse_train_writer = Data_Writer(folder, "mse_train.csv".format(folder), index=SNR_vect, columns=columns)
    ser_train_writer = Data_Writer(folder, "ser_train.csv".format(folder), index=SNR_vect, columns=columns)
    mse_test_writer = Data_Writer(folder, "mse_test.csv".format(folder), index=SNR_vect, columns=columns)
    ser_test_writer = Data_Writer(folder, "ser_test.csv".format(folder), index=SNR_vect, columns=columns)
    mc_report = MC_Report_Writer(folder, "report.json".format(folder), Nb_trials)
    mc_report.start()
    ...
    for SNR in SNR_vect:
        ...
        dataset.set_SNR(SNR)
        demux = Demux(N_vect)   # preamble configuration
    ...
    # save data
    mc_report.stop()
    print("file saved in the folder results/simulation3")
